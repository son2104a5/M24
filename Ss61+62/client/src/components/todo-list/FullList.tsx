...
export default function FullList({ jobs, openFormDelete, setStatus, openFormEdit}: Props) {
  return (
    <div className="overflow-auto max-h-[300px]">
        {
            jobs.map((job) => {
                return (
                    <div key={job.id} className="flex justify-between w-[500px] shadow-[0_3px_10px_rgb(0,0,0,0.2)] mb-[20px] p-2">
                        <div className="flex gap-2">
                            <input type="checkbox" checked={job.status} onChange={() => setStatus(job.id)}/>
